...
export default function PurchaseFrequencyDownloadButton({ startDate, endDate }: PurchaseFrequencyDownloadButtonProps) {
  const { fetchPurchaseFrequency, isDownloading } = usePurchaseFrequencyDownload()

  const handleDownload = async () => {
    try {
      const data = await fetchPurchaseFrequency(startDate, endDate)

      // 데이터 변환이 필요한 경우 여기서 처리합니다.
      // 현재 data는 { range: string, count: number }[] 형태이므로 바로 사용 가능합니다.
      downloadCSV(data, `purchase_frequency_${startDate || 'start'}_${endDate || 'end'}`)
    } catch (error) {
